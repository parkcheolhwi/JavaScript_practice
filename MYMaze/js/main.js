...
//           if(data[row][col] === 1){
//             this.ctx.fillRect(col * this.WALL_SIZE, row * this.WALL_SIZE, this.WALL_SIZE, this.WALL_SIZE);
//           }
//         }
//       }
//     }
...
(() => {
  ...
  class MazeRender{
    ...
    render(data){
      canvas.height = data.length * this.WALL_SIZE;
      canvas.width = data[0].length * this.WALL_SIZE;
      for (let row = 0; row < data.length; row = (row + 1) | 0){
        for(let col = 0; col < data[0].length; col = (col + 1) | 0){
          if(data[row][col] === 1){
            this.ctx.fillRect(col * this.WALL_SIZE, row * this.WALL_SIZE, this.WALL_SIZE, this.WALL_SIZE);
          }
        }
